// Upload the latest APK to a stable, public S3 key so the website can serve
// it from /api/download/android without any auth. Uses an admin presigned PUT
// + Node https stream (matching the proven upload-to-myspace path that works
// with Wasabi).
//
// Usage: node apps/web/scripts/upload-public-apk.js <apkPath>
...
if (!apkPath) {
  console.error('Usage: node upload-public-apk.js <apkPath>');
  process.exit(1);
}

const abs = path.resolve(apkPath);
if (!fs.existsSync(abs)) {
  console.error('APK not found:', abs);
  process.exit(1);
}

// Match @myphoto/shared exactly — Wasabi region is the bare name.
const REGION = 'eu-central-2';
const ENDPOINT = `https://s3.${REGION}.wasabisys.com`;
const BUCKET = process.env.WASABI_BUCKET || 'myphoto-prod';
const KEY = 'public/myphoto-android-latest.apk';
const CONTENT_TYPE = 'application/vnd.android.package-archive';

// The local .env stores the access key with a label prefix
// ("access-key= AKIA…") which the deployed Vercel env doesn't have. Strip
// any "<label>= " prefix so the local script signs requests with a clean id.
...
const s3 = new S3Client({
  ...
  forcePathStyle: true,
  // AWS SDK v3.729+ adds checksum params to presigned URLs that Wasabi
  // rejects; opt out so the presigned PUT actually works.
